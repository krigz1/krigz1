...
if (-not $UERoot) { $UERoot = $env:UE5_ROOT }
if (-not $UERoot) {
...
switch ($Target) {
  'editor' { $TargetName = 'LivingWorldMMOEditor' ; $ExtraArgs = '' }
  'game'   { $TargetName = 'LivingWorldMMO'       ; $ExtraArgs = '-game' }
  'server' { $TargetName = 'LivingWorldMMO'       ; $ExtraArgs = '-server -log' }
if (-not $UERoot) {
    $UERoot = $env:UE5_ROOT
}
...
Write-Host "[3/3] Running editor..."
& $EditorExe $UProject $ExtraArgs $RunArgs
